        ...
        if (createUser.signin_method == "google"):
            ...
            if ((idInfo['email_verified'] == True) and (idInfo['aud'] == settings.g_client_id)):
                ...
                phone_hash = hashlib.sha1(
                    createUser.primary_mobile_number.encode('utf-8')).hexdigest()

                user_payload.update(
                    {'email_hash': email_hash, 'primary_mobile_number': phone_hash})

        elif (createUser.signin_method == "firebase"):

            email_hash = hashlib.sha1(idInfo.get(
                'primary_email_id').encode('utf-8')).hexdigest()
            phone_hash = hashlib.sha1(idInfo.get(
