...
async def createDevice(payload):
    ...
    # create hardware device
        ...
        if (rs["RESULT"] == 1):
            response_data = {
                'code': error_msg_library.error_lib["device"]["DEVICE-101"]["code"],
                'message': error_msg_library.error_lib["device"]["DEVICE-101"]["message"],
            }
        else:
            response_data = {
                'code': error_msg_library.error_lib["device"]["DEVICE-104"]["code"],
                'message': error_msg_library.error_lib["device"]["DEVICE-104"]["message"],
            }
...
async def activateDeactivateDevice(payload):
    ...
    cur.callproc('sp_v3_activate_deactivate_device', [
                 payload["user_uuid"], payload["device_uuid"], payload["action"]])
    ...
    if ((payload["action"] == "activate") and (rs["RESULT"] == 1)):
        ...
        response_data = {
            'code': error_msg_library.error_lib["device"]["DEVICE-107"]["code"],
            'message': error_msg_library.error_lib["device"]["DEVICE-107"]["message"],
        }
    elif ((payload["action"] == "activate") and (rs["RESULT"] == 0)):

        response_data = {
            'code': error_msg_library.error_lib["device"]["DEVICE-108"]["code"],
            'message': error_msg_library.error_lib["device"]["DEVICE-108"]["message"],
        }
    elif ((payload["action"] == "deactivate") and (rs["RESULT"] == 1)):

        response_data = {
            'code': error_msg_library.error_lib["device"]["DEVICE-109"]["code"],
            'message': error_msg_library.error_lib["device"]["DEVICE-109"]["message"],
        }
    elif ((payload["action"] == "deactivate") and (rs["RESULT"] == 0)):

        response_data = {
            'code': error_msg_library.error_lib["device"]["DEVICE-110"]["code"],
            'message': error_msg_library.error_lib["device"]["DEVICE-110"]["message"],
        }
...
async def searchDevice(payload):
    ...
    cur.callproc('sp_v3_get_device', [
                 payload["device_uuid"], payload["user_uuid"]])
